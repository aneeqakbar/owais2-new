...
from scrapper.models import ScraperData, ScrapperOutput_withNumbers
...
class ScraperDashboardView(View):
    def get(self, request):
        if not request.session['is_login'] is True:
            return redirect("index")

        scraper_datas = [
            ScraperData.objects.filter(scraper_name="T", code="A"),
            ScraperData.objects.filter(scraper_name="T", code="B"),
            ScraperData.objects.filter(scraper_name="R"),
        ]

        context = {
            "scraper_datas": scraper_datas,
        }
        
        return render(request, "home/sheet-dashboard.html", context)
...
class ScraperDataView(View):
    def get(self, request, slug):
        ...
        if not request.session['is_login'] is True:
            return redirect("index")
        if not request.session.get('scraper_access', None) == f"{slug}{code}":
            context = {
                "slug": slug,
                "code": code,
            }
            return render(request, "home/scraper-login.html", context)

        scraper_name = [scraper_name[0] for scraper_name in ScraperData.SCRAPER_NAMES if scraper_name[1] == slug]
        if not scraper_name:
            raise Http404()

        queryset = ScraperData.objects.filter(scraper_name=scraper_name[0], code=code)
        ...
        context = {"data": data}
        return render(request, "home/tebular_data.html", context)
...
class ViewDataChartAnalytics(View):
    def get(self, request, data_id, field):
        days = request.GET.get("days", 0)
        template_name = 'home/chart_data.html'
        data = get_object_or_404(ScraperData, id = data_id)
